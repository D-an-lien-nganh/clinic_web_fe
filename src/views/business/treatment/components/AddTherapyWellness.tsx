...
export default function AddTherapyDisease({
  ...
  const findFirstPendingTreatmentSession = (selectedRecord: any) => {
    // Kiểm tra xem selectedRecord có tồn tại và có treatment_request không
    if (!selectedRecord || !selectedRecord.treatment_request) {
      return null;
    }

    // Tìm request TLDS
    const tldsRequest = selectedRecord.treatment_request.find(
      (request: any) => request.service_name === "TLDS"
    );

    // Kiểm tra xem tldsRequest có tồn tại và có treatment_sessions không
    if (!tldsRequest || !tldsRequest.treatment_sessions) {
      return null;
    }

    // Tìm phiên đầu tiên chưa hoàn thành
    const firstPendingSession = tldsRequest.treatment_sessions.find(
      (session: any) => !session.is_done
    );

    // Kiểm tra xem firstPendingSession có tồn tại không
  ...
  useEffect(() => {
    if (selectedRecord) {
      const filteredRequest = selectedRecord.treatment_request.find(
        (request: { service_name: string }) => request.service_name === "TLDS"
      );
                ...
                <Form.Item label="Kỹ thuật thực hiện">
                  <Table
                    dataSource={[
                      ...
                      ...(
                        form.getFieldValue("session_techical_settings") || []
